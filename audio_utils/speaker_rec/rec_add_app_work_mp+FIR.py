...
class MainWindow(QtWidgets.QMainWindow, design.Ui_MainWindow):
   def __init__(self):
      ...
      with open(self.mfcc_path, 'rb') as file:
          self.dataset_mfcc = pickle.load(file)
   ...
   def add_person(self):
      ...
      num_of_files = len(self.dataset_mfcc) + 1
      ...
      self.dataset_mfcc.append(self.mfcc(new_path))
                
      self.labels.append(full_name)
                
      with open(self.mfcc_path, 'wb') as file:
          pickle.dump(self.dataset_mfcc, file, pickle.HIGHEST_PROTOCOL)
                    
      with open(self.labels_path, 'wb') as file:
          pickle.dump(self.labels, file, pickle.HIGHEST_PROTOCOL)       
      output_str = "User " + full_name + " is added."
   ...
   def recognize_person(self):
      ...
      min_shape_mfcc = min((mfcc.shape[1]) for mfcc in self.dataset_mfcc)
      if (mfcc1.shape[1]<min_shape_mfcc):
          min_shape_mfcc = mfcc1.shape[1]
      
      pool = mp.Pool(processes=mp.cpu_count())
      pool_test_dataset = ((mfcc1[:, :min_shape_mfcc],
                             mfcc2[:, :min_shape_mfcc]) for mfcc2 in self.dataset_mfcc)
      distances = pool.map(acc_dtw.accelerated_dtw, pool_test_dataset)
      num_min = np.argmin(distances)
      
      if distances[num_min] > 2000:
          self.textEdit.append('Hallo, Guest.')
      
      for i in range (len(distances)):
          output_str = str(self.labels[i]) + " - " + str(distances[i])
          self.textEdit.append(output_str)
          
      output_str = "\n"+str(self.labels[num_min]) + " - " + str(distances[num_min])
      self.textEdit.append(output_str)
      pool.close()
